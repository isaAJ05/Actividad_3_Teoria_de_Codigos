    ...
    for perm in permutations(range(n)):  # Iterar sobre todas las permutaciones posibles de las columnas
        ...
        for escalares in product(range(1, q), repeat=n):  # Iterar sobre todos los escalares posibles
            ...
            if np.array_equal(G1_escalada, G2):  # Verificar si la matriz permutada es igual a G2
                ...
                # Verificar si hay multiplicaciones por escalares diferentes de 1
                for col, escalar in enumerate(escalares): # Iterar sobre las columnas y los escalares
...
while True:
    ...
    # Solicitar al usuario la longitud del código, la dimensión del código y la cardinalidad del alfabeton = int(input("->Digite la longitud del código (n): "))
    ...
    if permutaciones(Generadora1, Generadora2, q): # Llama a la función permutaciones
        print("") # Si es True (Equivalente), que imprima lo correspondiente en su función
    else: # En caso de que no sean equivalentes
