...
def verificar_permutacion(G1, G2):
    ...
    for perm in permutations(range(n)):
        ...
        if np.array_equal(G1_permutada, G2):
            # Identificar las columnas que se intercambian, ajustando los índices para que arranquen desde 1
            intercambios = [(i + 1, perm[i] + 1) for i in range(n) if i != perm[i]]
            return True, intercambios
    return False, None
...
print(Generadora2)
print("\n Codewords G1")
codewords_G1 = hallar_codewords(Generadora2, q, k)
print("→ C1= ", codewords_G1)
print("\n Codewords G2")
codewords_G2 = hallar_codewords(G2, q, k)
print("→ C2= ", codewords_G2)

equivalentes, intercambios = verificar_permutacion(G1, G2)
if equivalentes:
