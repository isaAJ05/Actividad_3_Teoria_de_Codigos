    ...
    for perm in permutations(range(n)):  # Iterar sobre todas las permutaciones posibles de las columnas
        ...
        for escalares in product(range(1, q), repeat=n):  # Iterar sobre todos los escalares posibles
            ...
            if np.array_equal(G1_escalada, G2):  # Verificar si la matriz permutada es igual a G2
                ...
                if intercambios!='' and operaciones!=[]: # Si hay permutación entre proposiciones y operaciones con escalares, imprimirlos.
                ...
                elif operaciones!=[]: # Si hay operación con escalares, imprimirlas.
                    for operacion in operaciones:
                        print(operacion)
                     # Codewords
                    codewords_G1 = hallar_codewords(Generadora1, q, k)
                    ...
                    print(f'C2={{{codewords_G2}}}')
                    return es_equivalente
                elif intercambios!='': # Si hay permutación entre proposiciones, decir los intercambios.
                    print("\nPermutación entre posiciones:", intercambios)
                     # Codewords
                    codewords_G1 = hallar_codewords(Generadora1, q, k)
                    print(f'C1={{{codewords_G1}}}')
...
def resultados_permutaciones(intercambios, operaciones): # Imprimir resultados de las permutaciones
  if intercambios!='': # Si hay permutación entre proposiciones, decir los intercambios.
      print("\nPermutación entre posiciones:", intercambios)
  else:
      print("\nLas matrices generadoras no son equivalentes mediante permutación entre posiciones.")

  if operaciones!=[]: # Si hay operación con escalares, imprimirlas.
    for operacion in operaciones:
        print(operacion)
  else:
      print("\nNo se encontraron operaciones entre escalares para convertir G1 en G2.")
...
print("\nMatriz G2:")
print(Generadora2)
permutaciones(Generadora1, Generadora2,q)
if not permutaciones(Generadora1, Generadora2,q):
