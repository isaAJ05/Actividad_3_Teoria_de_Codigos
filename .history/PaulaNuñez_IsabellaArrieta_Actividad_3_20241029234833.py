...
def son_equivalentes(codewords1, codewords2):
    n = len(codewords1[0])
    for perm in permutations(range(n)):
        permutados = [list(np.array(cw)[list(perm)]) for cw in codewords1]
        if sorted(permutados) == sorted(codewords2):
            return True, perm
    return False, None
...
equivalentes, permutacion = son_equivalentes(codewords_G1, codewords_G2)
if equivalentes:
    print("\nLos códigos son equivalentes mediante la permutación de columnas:", permutacion)
else:
    print("\nLos códigos no son equivalentes mediante permutaciones de columnas.")